...
def load_settings() -> models.BridgeRequest:
    ...
    try:
        ...
        if content != dataclasses.asdict(config):
            ...
            shutil.copy(bridge_settings_file_full_path, str(CONFIG_DIR / backup))
            save_settings(config)
            LOGGER.info(f"settings.yml schema was updated and the previous settings2.yml was backed up: {backup}", None,
                        True)
