...
def process_args():
    ...
    group.add_argument(f"--kill", help="Kill a bridge container", action='store_true')
    ...
    parser.add_argument(f"--token", help ="Specify a token name to use from tokens.yml for the --run or --kill commands", type=str)
    ...
    if args.build:
        ...
        BridgeContainerBuilder(LOGGER, req).build_bridge_image(True)
    elif args.push_image:
        push_bridge_image()
    elif args.run or args.kill:
        if not args.token:
            k = "run" if args.run else "kill"
        ...
        if args.run:
            ...
            container_name = BridgeContainerBuilder.get_bridge_container_name(token.sitename, token.name)
            DockerClient(LOGGER).kill_container(container_name)
    elif args.update:
        ...
        BridgeStatusLogic(LOGGER).remove_agent(token, args.agent_name, agent_sitename, LOGGER)
