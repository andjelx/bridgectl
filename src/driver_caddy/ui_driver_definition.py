...
class UiDriverDefinition:
    ...
    def define_drivers(req):
        ...
        st.markdown("---")
        selected_only_db_drivers = st.checkbox("Build image with only drivers, no bridge rpm", value=req.bridge.only_db_drivers)
        if selected_only_db_drivers != req.bridge.only_db_drivers:
            req.bridge.only_db_drivers = selected_only_db_drivers
            bridge_settings_file_util.save_settings(req)

    @staticmethod
    @st.dialog("Upload Driver Setup Files", width="large")
    def upload_driver_setup_files():
        path_short = StringUtils.remove_before(str(bridge_container_builder.driver_files_path), "bridgectl/")
        ...
        driver_file = st.file_uploader("", accept_multiple_files=False)
        if driver_file:
            if not bridge_container_builder.driver_files_path.exists():
                bridge_container_builder.driver_files_path.mkdir(parents=True)
            driver_path = bridge_container_builder.driver_files_path / driver_file.name
