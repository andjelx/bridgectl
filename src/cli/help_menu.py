...
class HelpMenuQuestion(Question):
    ...
    def ask(self):
        if self.show_welcome:
            ...
            LOGGER.info_rich(f"A command-line utility for deploying, monitoring, and troubleshooting Tableau Bridge.", style="green", highlight=False)
            LOGGER.info_rich(f"Current local directory: {os.getcwd()}", style="green", highlight=False)
            LOGGER.info_rich(f"Local python version: {python_version()} from {sys.executable}", style="green", highlight=False)
            LOGGER.info_rich(f"Downloaded from: {GithubVersionChecker().get_releases_home()}, Deployment Env: {APP_CONFIG.deployment_environment}",style="green", highlight=False)
        try:
            choices = [
