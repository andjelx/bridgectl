...
class DriverScriptGenerator:
    def __init__(self, logger: LoggerInterface, buildimg_path):
        self.logger = logger
        self.db_aliases = {}
        self.drivers_def = []
    ...
    def gen(self, driver_names: list, linux_distro, use_download_cache: bool):
        ...
        # STEP - Prep
        self.pre_post_scripts, self.drivers_def = DriverDefLoader(self.logger).load_driver_defs()
        buildimg_driver_path = Path(self.buildimg_path) / "drivers"
        if not buildimg_driver_path.exists():
            buildimg_driver_path.mkdir()
        ...
        for driver_name in driver_names:
            ...
            driver_name = driver_def.download_url.split('/')[-1]
            dl_path = buildimg_driver_path / driver_name
            download_out += f'# Driver {driver_name} downloaded from {durl}\n'
            if durl.startswith(DriverKeywords.LOCAL):
                driver_name = driver_def.download_url.replace("LOCAL", "").lstrip()
                dl_path = buildimg_driver_path / driver_name
                if not dl_path.exists():
                    w = f", you can download from website {driver_def.website}" if driver_def.website else ""
                    err = f'Local driver file for `{driver_def.driver}` not found locally at {dl_path}{w}'
                ...
                if use_download_cache:
                    if not dl_path.exists():
                        ...
                        s3_client.download_file(durl, buildimg_driver_path) # if local file doesn't already exist, then download it from S3
                else:
                ...
                if use_download_cache:
                    if not dl_path.exists():
                        self.logger.info(f'downloading driver from web: {durl} to {buildimg_driver_path}')
                        download_util.download_file(durl, str(dl_path))
                    else:
                        # just show filename, not full path
                        self.logger.info(f'Using cached driver: {dl_path}')
                else:
                    download_out += f"curl -sS --location --output /tmp/driver_caddy/{driver_name} {driver_def.download_url}\n"
            else:
            ...
            # website = "" if not driver_def.website else f"   website: {driver_def.website}"
            download_out += f"[ -f /tmp/driver_caddy/{driver_name} ] || exit 1" + "\n\n"
            copy_driver_files.append(driver_name)

            # STEP - install script
            v = driver_def.version if driver_def.version else ""
            install_out += f'# Install {driver_name}   {v}\n'
            if driver_def.type == DriverType.jar:
                install_out += f"mv -f /tmp/driver_caddy/{driver_name} /opt/tableau/tableau_driver/jdbc/{driver_name}\n\n"
            elif driver_def.type == DriverType.install:
                install_out += self.replace_variables(driver_def.install, {"$download_name": driver_name}) + "\n"
                # script_i += "# test\n" + driver_def['test']
            else:
    ...
    def script_path_buildimg(self):
        path_download_drivers = Path(self.buildimg_path) / 'drivers_download.sh'
        path_install_drivers = Path(self.buildimg_path) / 'drivers_install.sh'
