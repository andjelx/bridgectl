...
class EditAppSettings(Question):
    ...
    m_edit_streamlit_server_address = "Edit web UI server address"
    ...
    def ask(self):
        try:
            ...
            LOGGER.info("Use this wizard to edit app settings")
            LOGGER.info(f"You probably want to change the streamlit server address to the local hostname: {hostname}")
...
def edit_streamlit_server_address():
    ...
    hostname = MachineHelper.get_hostname()

    LOGGER.info_rich(f"Please provide the address for the BridgeCTL Web User Interface. Typically, this is either 'localhost' or the hostname which is '{hostname}'")
    while edit:
...
def configure_systemctl_service():
    ...
    LOGGER.info("Note: you may also want to set the UI server address to the hostname instead of localhost.")
