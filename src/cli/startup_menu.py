...
def print_welcome_message(show_logo: bool):
    ...
    LOGGER.info_rich("Bridge CTL - A utility to build, run and monitor Tableau Bridge Agents in Containers", style="green", highlight=False)
    gh = "" if APP_CONFIG.is_devbuilds() else "github.com/"
...
def run_streamlit():
    ...
    if proc_pid:
        LOGGER.info(f"  UI is already running with pid {proc_pid}")
...
# def run_streamlit_wh():
#     script_name = "streamlit run Home.py"
#     cmd = "export PYTHONPATH=.. && "
#     cmd += f'"{sys.executable}" -m {script_name}'
#     LOGGER.info(f"Executing command: {cmd}")
#     # LOGGER.info(f"url: {url}")
#     working_dir = pathlib.Path(__file__).parent.parent.parent / "warhammer"
#     process = SubProcess.run_popen(cmd, working_dir)
...
class StartMenuQuestion(Question):
    ...
    # m_start_warhammer_ui = "Start Warhammer UI"
    m_edit_app_settings = "Edit App Settings"
    m_show_help = "Help ->"
    m_quit = "Quit"
        ...
        try:
            {
                ...
                # self.m_start_warhammer_ui: run_streamlit_wh,
