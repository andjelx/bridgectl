...
from src.enums import ImageRegistryType, RunContainerAsUser, VALID_DOCKER_NETWORK_MODES, \
    DEFAULT_DOCKER_NETWORK_MODE, PropNames, DEFAULT_POOL
...
class BridgeContainerRunner:
    ...
    def run_bridge_container_in_docker(self, app: AppSettings = None):
        ...
        if app.img_registry_type == ImageRegistryType.aws_ecr:
            reg = EcrRegistryPrivate(self.logger, app.ecr_private_aws_account_id, app.ecr_private_repository_name, app.aws_region)
            local_image_id = reg.pull_image(app.selected_image_tag)
            ...
            if not img:
                self.logger.info(f"Bridge image named {app.selected_image_tag} is missing. Please Build first.")
                return
            else:
                local_image_id = img.id
        ...
        bridge_container_name = BridgeContainerBuilder.get_bridge_container_name(self.token.sitename, self.token.name)
    ...
    def remove_bridge_container_in_docker(logger, bridge_container_name):
        ...
        docker_client.kill_container(bridge_container_name)
        ...
        logger.info(f"calling Tableau Cloud API to remove agent")
